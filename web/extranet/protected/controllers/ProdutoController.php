...
class ProdutoController extends GxController {

	
    public function getRepresentingFields(){
		return Produto::representingColumn();
	}
    
	public function actionView($id) {
		$this->render('view', array(
			'model' => $this->loadModel($id, 'Produto'),
		));
	}
	...
	public function actionCreate() {
		...
		if (isset($_POST['Produto'])) {
			$model->setAttributes($_POST['Produto']);

			if ($model->save()) {
				if (Yii::app()->getRequest()->getIsAjaxRequest())
					Yii::app()->end();
				else
					$this->redirect($this->createUrlRel('view',array('id' => $model->idproduto,'success'=>'create')));
			}
		}
        else{
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Produto'])) {
			...
			if ($model->save()) {
                $this->redirect($this->createUrlRel('view',array('id' => $model->idproduto,'success'=>'update')));
			}
		}
